<template>
  <div id="login">
    ...
    <div class="login-connect">
      ...
      <div class="loginBtn">
        <div class="submit" @click="login"></div>
      </div>
    </div>
    ...
    <div class="register-connect">
      ...
      <div class="gender">
        <span @click="optionShow=!optionShow">{{gender | genderFormat}}</span>
        <div class="option" v-if="optionShow">
          <div @click="()=>{gender=0;optionShow=false}">保密</div>
          <div @click="()=>{gender=1;optionShow=false}">女</div>
          <div @click="()=>{gender=2;optionShow=false}">男</div>
        </div>
      </div>
      <div class="motto">
        <textarea v-model="motto" cols="20" rows="2" placeholder="留言板"></textarea>
      </div>
      <div class="registerBtn">
        <div class="submit" @click="register"></div>
      </div>
    </div>

    <div class="footer">
      <p>
        <span class="forget">忘了密码?</span>
        <span class="line"></span>
        <span class="singup" @click="goRegister">注册</span>
      </p>
      <p>登录即同意《用户协议》及《营私协议》</p>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    $("#login").animate({ left: "0%" }, 500, function() {});
  },
  filters: {
    genderFormat(value) {
  ...
  methods: {
    goRegister() {
      this.username = "";
      this.password = "";
      $(".login-connect").animate({ top: "-100%" }, 300, function() {});
      $(".register-connect").animate({ top: "50%" }, 300, function() {});
    },
    goLogin() {
      this.username = "";
      this.password = "";
      this.gender = 0;
      this.motto = "";
      $(".register-connect").animate({ top: "200%" }, 300, function() {});
      $(".login-connect").animate({ top: "50%" }, 300, function() {});
    },
    back() {
      const self = this;
      $("#login").animate({ left: "-100%" }, 500, function() {
        self.$store.commit("reverseShowLogin", false);
      });
    },
    login() {
      const self = this;
      login(this.username, this.password).then(data => {
        if (data.ok) {
          self.$store.commit("setUserId", data._id);
          $("#login").animate({ left: "-100%" }, 500, function() {
            self.$store.commit("reverseShowLogin", false);
          });
        } else {
...
.login-connect input::-webkit-input-placeholder {
  color: #fff;
  text-align: left;
}
...
.register-connect input::-webkit-input-placeholder {
  color: #fff;
  text-align: center;
}

.register-connect .motto > textarea {
  width: 100%;
  height: 4rem;
  padding: 0.5rem;
  position: absolute;
...
.register-connect .motto > textarea::-webkit-input-placeholder {
  color: #fff;
}
